...
- 状态：v0.4.1 代码完成但发布闭环进行中；完成本机 8899 验证与 GitHub 发布后才进入 v0.4.2
- 基线版本：`v0.3.4`
...
- 状态：代码完成；未单独发布，纳入当前待发布的 v0.4.1 累积版本
...
- 状态：进行中；代码完成，待本机 8899 验证与 GitHub 发布闭环
...
- 未提交、未推送、未打标签、未发布。
- 当前沙箱禁止真实 TCP 监听，因此使用 handler 级 API 冒烟；迁移和全部业务测试不受影响。
- 多目标重复基础探测、任务持久化和调度恢复分别留给 v0.4.2、v0.4.1。

下一步：

- `v0.4.1 / V041-MIGRATION`：新增 `job_runs` 与 `scheduler_state`，先写持久化和重启中断测试。

日期：2026-07-10  
版本：v0.4.1  
状态：进行中；代码完成，待本机 8899 验证与 GitHub 发布
...
- 未提交、未推送、未打标签、未发布。
- 真实 TCP 监听继续受当前沙箱限制，API 使用 handler 级测试。
- 不支持网络任务断点续跑、多进程抢锁或分布式协调。

实时断点：

- 最近完成：新增三项长期硬性要求并同步 AGENTS、接手文档、路线图和部署说明。
- 正在执行或准备执行：备份数据，更新并重启现有 `127.0.0.1:8899` 服务，完成本地验收后执行 v0.4.1 GitHub 发布闭环。
- 当前阻塞：本次用户请求仅要求修改指导文档，因此尚未执行部署、提交或发布。

下一步：

- 完成 v0.4.1 的本机 8899 更新与 health/login/bootstrap/API/Web/网关冒烟，然后 commit、push、annotated tag、GitHub Release、资产与 GHCR 检查；全部成功后开始 `v0.4.2 / V042-CHECK-PLAN`。
