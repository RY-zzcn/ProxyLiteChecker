...
- 当前代码版本：`v0.4.3`
- 当前已发布版本：`v0.4.2`
- 当前唯一开发阶段：`v0.4.3 / GITHUB-RELEASE`
- 下一开发版本：`v0.4.3`
- 路线图终点：`v0.4.3`
- 最后校准日期：2026-07-10
- 已发布版本提交：`6a74ec872117f698984f9e490f06448cedca7999`（v0.4.2）
- 当前工作区基线：`main` 提交 `abd22d1` 加 v0.4.3 未提交实现；不得覆盖或拆分当前工作区

本文是后续 Codex 会话的第一份项目状态入口。已完成的数据模型与性能路线见 [ROADMAP_V0.4.0_TO_V0.4.2.md](ROADMAP_V0.4.0_TO_V0.4.2.md)，当前部署阶段见 [ROADMAP_V0.4.3.md](ROADMAP_V0.4.3.md)。
...
v0.4.2 的实现、自动化、本机 8899 验收和 GitHub 发布均已完成。用户已明确启动 v0.4.3：新增交互式一键部署脚本，提供 GitHub Release 二进制和 GHCR Docker 两种方式，默认安装到 `/opt/ProxyLiteChecker`，严格清理临时文件并重写 README。
...
| `v0.4.3` | 实现与 8899 验收完成，GitHub 发布中 | 一键部署 | 二进制/Docker 交互安装、Docker 环境处理、`/opt` 规范目录和简化文档 |
